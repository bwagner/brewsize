...
def get_json_info(package):
    ...
    """
    proc = subprocess.Popen(['brew','info', '--json=v1', package],stdout=subprocess.PIPE)
...
def get_size(package, indent=0):
    ...
    if installed:
        ...
        logger.debug( "{}{} not installed, checking deps: {}".format(
            ind, 
            package, 
            ", ".join(map(str, dependencies)) if len(dependencies) > 0  else "None."
        ))
        resp = requests.head(url, allow_redirects=True);
        size = int(resp.headers['Content-Length'])
        if len(dependencies) == 0:
            logger.debug( "{}{} size {} ({})".format(ind, package, size, sizeof_fmt.sizeof_fmt(size)))
        else:
            logger.debug( "{}{} alone size {} ({})".format(ind, package, size, sizeof_fmt.sizeof_fmt(size)))
            size += reduce(lambda x,y : x+y, map(lambda x: get_size(x, indent+1), dependencies), 0)
            logger.debug( "{}{} with deps size {} ({})".format(ind, package, size, sizeof_fmt.sizeof_fmt(size)))
...
def make_args(pgnam):
    ...
    parser.add_argument("-l", "--loglevel", help="set loglevel to one of ERROR, CRITICAL, WARNING, DEBUG, INFO")
...
def main():
    ...
    if packagesizeinbytes == 0:
        ...
        get_logger().info("{} package size including dependencies: {}".format(brew_package, sizeof_fmt.sizeof_fmt(packagesizeinbytes)))


# curl -I -L "$(brew info --json=v1 $@ | jq -r '.[0].bottle.stable.files.el_capitan.url')" 2>&1 | egrep '^Content-Length: [^0]' | cut -d ' ' -f 2
